...
#include<iostream>
#include<algorithm>    //for sorting array with less time complexity
#include<cmath>
using namespace std;
...
int main(){
    ...
    float tempresult; 
    for(int i =0;i<N;i++){
        count = 0; // reset count at start of each i loop

        for(int j =0;j<N;j++){
            tempresult = (float)arr[j] / arr[i]; //  force float division
            count += ceil(tempresult);
        }

        if(count<= D){
            cout<<"result : "<<arr[i]<<endl; //  was arr[i+1], caused out of bounds
            break;
        }
    }
    return 0;
